...
def data_summary(data):
    summary = dict()
    summary["len"] = len(data['train'])+len(data['test'])
    summary["n_slices"] = np.concatenate( (np.array(list(map(lambda x: len(x[0]), data['train']))),
                                          np.array(list(map(lambda x: len(x[0]), data['test'])))) )
    summary["n_features"] = n_features
    summary["max_seq_len"] = summary["n_slices"].max()
    summary["cancer"] = np.concatenate( (np.array(list(map(lambda x: x[1][1], data['train']))),
                                         np.array(list(map(lambda x: x[1][1], data['test'])))) )
...
class DataBatchGenerator(object):
    ...
    def next_train(self, batch_size):
        ...
        batch_seqlen = []
        for i in batch_id:
            batch_images.append(self.train_data[i][0])
            batch_labels.append(self.train_data[i][1])
    ...
    def get_test(self):
        ...
        test_seqlen = []
        for i in self.test:
            test_images.append(self.test_data[i][0])
            test_labels.append(self.test_data[i][1])
...
def wrapper(learning_rate=1000, n_embedding=256, n_hidden=100, penalty_scale=500):
    learning_rate = learning_rate * 0.0005
    penalty_scale = penalty_scale * 0.00025
    n_embedding = n_embedding * 15
    n_hidden = n_hidden * 10

    bottle_neck_data = np.load(
        "/labs/colab/3DDD/kaggle_data/kaggle_processed_data/resampled_data_new_divided.npy")
    bottle_neck_data = bottle_neck_data.item()
